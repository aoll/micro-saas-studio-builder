...
// (TOOLING-test-transaction): every test that writes now runs inside a
// transaction that is always rolled back, so the manual afterAll deletes
// this file used to track (createdUserIds, createdProductIds) are gone.
// Assertions are unchanged (the explicit-timestamp switch above landed in
...
describe("listPurchases", () => {
  ...
  it("returns the caller's purchases newest first via the real purchase(), with only the documented fields", async () => {
    await withTestTransaction(async () => {
      ...
      // twice, then backdate the older one directly. Two `now()` calls
      // inside the same transaction return the same instant (now() is
      // frozen for a whole Postgres transaction, TOOLING-test-transaction),
      // so the `setTimeout` this test used to rely on no longer produces
      // distinct timestamps once it runs inside withTestTransaction.
