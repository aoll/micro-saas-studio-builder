...
// Moved here from app/(products)/[app]/not-found.test.tsx when the SA-08
// content was extracted into this shared component (plan round 2, task
// R2): every case kept, none weakened, only the import target changed.
describe("ProductNotFound", () => {
  it("shows the closed message with the product's name for a killed product", async () => {
    ...
    const { ProductNotFound } = await import("./product-not-found");
  ...
  it("shows the generic message for an unknown slug, never echoing the requested slug", async () => {
    ...
    const { ProductNotFound } = await import("./product-not-found");
    const { container } = render(await ProductNotFound());
