...
- `proxy.ts` (ex-`middleware.ts`) sert au bonus des sous-domaines (simple réécriture) et, depuis QA1-P1-B4, à poser le cookie `anonymous_id` sur la première requête GET d'une page produit, avant que `<TrackVisit>` n'envoie son beacon : jamais d'accès base ni de contrôle d'autorisation dedans.
