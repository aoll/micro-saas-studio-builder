...
// SA-08: rendered for an unknown slug (`getProduct` finds nothing) or a
// `killed` product (specs/SA-08-introuvable.md); also reached for a missing
// sub-page under an *active* product (e.g. `notFound()` from a page deeper
// in the tree), a third state this component must tell apart from the
// first two. The layout's own `notFound()` call (task 1's spike, outcome
// C) never actually renders this file for the product-level cases in the
// current build — see app/(products)/[app]/layout.tsx's comment and the PR
// body — but the component is still delivered so the blocker only has to
// be lifted, not designed, once resolved.
