...
- Every changed file is inside `Périmètre`:
  `git diff --name-only "origin/$(git config msb.integration)"...HEAD`
...
- No committed test was weakened: every commit that edits an existing test
  says why in its message (`git log -p "origin/$(git config msb.integration)"..HEAD -- '*.test.ts' '*.test.tsx'`)

## Phase 3 — Diff hygiene

```bash
git diff "origin/$(git config msb.integration)"...HEAD
```
