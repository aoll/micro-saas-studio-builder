...
const { SignupFlow } = vi.hoisted(() => ({
  SignupFlow: vi.fn((props: { slug: string; freeCreditsOnSignup: number; expired?: boolean }) => (
...
describe("SignupPanel", () => {
  it("passes expired=false when there is no error search param", async () => {
    ...
    const ui = await SignupPanel({ slug: "lettre-pro", freeCreditsOnSignup: 3, searchParams: Promise.resolve({}) });
    render(ui);

    expect(screen.getByTestId("signup-flow").dataset.expired).toBe("false");
    expect(SignupFlow).toHaveBeenCalledWith(
      expect.objectContaining({ slug: "lettre-pro", freeCreditsOnSignup: 3, expired: false }),
      undefined,
    );
  });
