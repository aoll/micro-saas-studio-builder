...
- Branch name: `git diff --name-only "origin/$(git config msb.integration)"...$ARGUMENTS`.
- Empty: `git diff --name-only --merge-base "origin/$(git config msb.integration)"` plus unstaged and untracked files.
