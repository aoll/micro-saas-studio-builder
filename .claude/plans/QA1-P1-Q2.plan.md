...
- **Security review follow-up 1 (HIGH, revises the "no DAL/contract change" scope, not the DAL itself)**: the *automatic* trigger — unlike the explicit magic-link path it reuses — is a brand-new, unauthenticated-by-URL mutation surface reachable from any page of any product, so it now calls `guardRequest("signup")` at the top of the `claimSignupBonus` Server Action, before delegating to the helper, exactly like `requestMagicLink` (`signup/_actions.ts`) and `purchase` (`checkout/_actions.ts`). A guard refusal returns `{ ok: false }` and grants nothing; the client leaf's existing swallow-and-retry behavior already covers this outcome without change.
- **Security review follow-up 2 (MEDIUM, revises the "Admin/owner sessions" note above)**: on reflection that note undersold the automatic-trigger case specifically — the *explicit* `/{slug}/signup/complete` magic-link path keeps accepting any signed-in user (unchanged, `route.test.ts` unmodified), but the *automatic* per-page-view trigger now checks `session.user.role === 'user'` in `CrossProductSignupBonus` and renders nothing otherwise. Rationale: an admin or owner browsing sub-app pages during a demo (shared Better Auth session, docs/01's "Mode démo public") would otherwise silently accumulate signup bonuses and `signup` events on every product visited, polluting the funnel with accounts never meant to buy anything.
